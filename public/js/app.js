...
// ── Firestore loader with graceful fallback ──────────────────
...
function renderProjects(projects) {
  ...
  grid.innerHTML = projects.map((p, i) => `
    <div class="proj-card rev-u" style="--cg:${GRADIENTS[p.color]||'var(--gradient)'}; transition-delay:${i*0.12}s">
      <div class="proj-head">
        <div class="proj-icon">${p.icon || '💻'}</div>
        <div class="proj-links">
          ${p.github ? `<a href="${p.github}" target="_blank" class="proj-link" title="GitHub"><i class="fab fa-github"></i></a>` : ''}
          ${p.live   ? `<a href="${p.live}"   target="_blank" class="proj-link" title="Live"><i class="fas fa-external-link-alt"></i></a>` : ''}
        </div>
      </div>
      <div class="proj-body">
...
async function init() {
  ...
  checkAdminStatus();
  setupReveal();
  loadProfileData(); // Load all profile fields from Firestore in background

  const [skills, projects, experience, certs] = await Promise.all([
    loadCollection('skills',       DEFAULTS.skills),
    loadCollection('projects',     DEFAULTS.projects),
    loadCollection('experience',   DEFAULTS.experience),
    loadCollection('certificates', DEFAULTS.certificates),
  ]);
